...
/*
  ...
  */
	static uint8_t cd = 128;

	for (uint16_t i=0; i<0x200; i++)
	{
		  USART_Rx_Buffer[USART_Rx_ptr_in] = cd;
		  cd+=4;
		  USART_Rx_ptr_in++;
	...
	//bf_ptr = USART_Rx_ptr_in;
	//cd+=8;
	//if (cd > 25) cd = 0;
}

/*******************************************************************************
*	Adds symbols to output buffer
*
*******************************************************************************/
void Add_Byte_To_OUT_Buff(char sym)
{
	USART_Rx_Buffer[USART_Rx_ptr_in] = sym;
	USART_Rx_ptr_in++;
